...
public class ManCustomerAddController implements Initializable{
	...
	public void add(ActionEvent e)
	{
		...
		if(passE.getText().equals("*MEDIUM")||userN.getText().equals("manager")||passE.getText().equals("*NOT STRONG")||!userN.getText().chars().allMatch(Character::isLetterOrDigit)||!custFirst.getText().chars().allMatch(Character::isLetter)||!custLast.getText().chars().allMatch(Character::isLetter))
		{
			...
			a.setHeaderText("Invalid Inputs!!!,also make sure that user name isnt manager");
			a.showAndWait();
		}
		...
		else
		{
			...
			if(Main.res.addCustomer(c))
			{
				...
				clear1();
				Alert a=new Alert(AlertType.CONFIRMATION);
				a.setHeaderText("Succeffully added customer!");
				a.showAndWait();
			}
			else
			{
	...
	//choose photo and convert it to byte array in order to save it.
