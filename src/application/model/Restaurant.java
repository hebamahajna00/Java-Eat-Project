...
public class Restaurant implements java.io.Serializable {
	...
	public boolean addDelivery(Delivery delivery) {
		try {
			...
			RegularDelivery rd = (RegularDelivery)delivery;
			delivery = new ExpressDelivery(rd.getDeliveryPerson(), rd.getArea(),rd.getIsDelivered(), rd.getOrders().first() ,rd.getDeliveredDate());
			int max=-1;
			...
			if(delivery instanceof RegularDelivery) {
				...
				for(Order o: rg.getOrders()) {
					...
					if(orders == null) {
						orders = new TreeSet<>(new SeriaComparator());
					}
					orders.add(o);
   ...
   class SeriaComparator implements Comparator<Order>,Serializable
   {
	...
	public boolean removeDeliveryPerson(DeliveryPerson dp) throws HasNoAreaException
	{
		if(dp.getArea()==null)
		{
			throw new HasNoAreaException();
		}
	...
	public boolean removeDelivery(Delivery delivery) throws HasNoAreaException
	...
	public ArrayList<Order> deliver(Delivery d) {
	...
	public TreeSet<Delivery> getDeliveriesByPerson(DeliveryPerson dp , int month){
		...
		for(Delivery d: getDeliveries().values()) {
			if(d.getDeliveryPerson()!=null&&d.getDeliveryPerson().equals(dp) && d.getDeliveredDate().getMonthValue() == month)
				delivered.add(d);
		}
		return delivered;
	}
